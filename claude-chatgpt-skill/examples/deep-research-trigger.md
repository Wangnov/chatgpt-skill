...
- **工具**：必须显式激活 Deep Research 模式，不能只在普通 composer 里写 "deep research"。入口可能是 `+` / `添加文件等` 菜单里的 **"深度研究"**，也可能是在 composer 输入 `@深度研究` 后选择候选。激活后 composer placeholder 会变成 **"获取详细报告"**，并出现 `深度研究` chip、`应用` 按钮、`推荐` / `报告` 等 DR 专属控件
- **预期产物**：30 分钟到几小时的长报告
...
2. 激活 Deep Research：
   - 优先用当前可见入口：点 composer 的 `+` / `添加文件等` 按钮 → "深度研究"
   - 如果菜单里没有，或当前页面只在候选里暴露该能力，在 composer 输入 `@深度研究` → 等候选出现 → 选中"深度研究"
3. **验证 DR 模式激活**：
...
| 信号 | 含义 | 处理 |
|---|---|---|
| Composer placeholder = "获取详细报告" + `深度研究` chip / `应用` / `推荐` / `报告` 等控件 | DR 模式已激活 | OK，可以发 |
