...
> Watcher 和主 Claude 都可能在任何时刻撞上下面这些状态。**永远不要静默重试**，永远把你看到的可见文本原样报告给用户，让他决定下一步。
...
- Watcher 立刻返回 NEEDS_USER_INPUT，blocker_type: captcha
- 让用户在浏览器里亲自完成验证，完成后让他通知主 Claude 重启 watcher
...
- Watcher 返回 NEEDS_USER_INPUT，blocker_type: oauth
...
- Watcher 返回 NEEDS_USER_INPUT，blocker_type: login
- 让用户在浏览器里亲自登录
- **绝对不要替用户输入密码 / OTP**
- 登录完成后让他重启 watcher
...
- Watcher 返回 NEEDS_USER_INPUT，blocker_type: continue_generating
- 让用户决定：要不要让 ChatGPT 续、是否已经够了
- **不要 watcher 自己点 Continue**——这是用户级别决策，可能再续几千 token 才停
...
- Watcher 返回 NEEDS_USER_INPUT，blocker_type: clarification_question
- blocker_visible_text 引用 ChatGPT 的原问题
- **不要替用户回答**——这违背委托的初衷，用户委托是希望 ChatGPT 帮他想，不是让 Claude 替他做选择
- 让用户回答后重启 watcher
...
- 这种在**提交前**就能看到，watcher 不会处理这个
- 主 Claude 在 send 之前看 chip 状态，发现失败就报告：原文件名 + 错误文本 + 文件大小
- 让用户决定：换文件、改格式、压缩
...
- 页面长时间无响应（read_page 多次失败）
- 整页 5xx 错误
- 网络面板大量失败请求（但 watcher 看不到这层）

### 处理

- Watcher 的异常处理路径：read_page 失败重试 1 次后仍失败 → 返回 ERROR
...
| 现象 | 默认动作 |
|---|---|
| 可重试的瞬时错误（网络抖动） | watcher 自动重试 1 次 |
